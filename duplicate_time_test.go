...
func TestPushDuplicateTime(t *testing.T) {
	...
	dumped := h.Dump()
	t.Logf("%v", dumped)
	if dumped[0].Payload != "task1" {
		t.Errorf("element 0 has wrong payload %s", dumped[0].Payload)
	}
	if dumped[1].Payload != "task2" {
		t.Errorf("element 1 has wrong payload %s", dumped[0].Payload)
	}
