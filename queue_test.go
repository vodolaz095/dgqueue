...
func TestQueue_Push(t *testing.T) {
	...
	if testQueue.tasks[0].Payload != "Task 1" {
		t.Error("wrong payload for 1st task")
	}
	if testQueue.tasks[1].Payload != "Task 2" {
		t.Error("wrong payload for 2nd task")
	}
	if testQueue.tasks[2].Payload != "Task 3" {
...
func TestQueue_Swap(t *testing.T) {
	testQueue.Swap(0, 1)
	if testQueue.tasks[0].Payload != "Task 2" {
		t.Error("wrong payload for 1st task")
	}
	if testQueue.tasks[1].Payload != "Task 1" {
		t.Error("wrong payload for 2nd task")
	}
	if testQueue.tasks[2].Payload != "Task 3" {
		t.Error("wrong payload for 3rd task")
