...
func TestHandlerPublishOK(t *testing.T) {
	...
	if task.Payload != "" || !task.ExecuteAt.IsZero() {
		t.Error("task returned")
	}
	tasks := h.Dump()
	if len(tasks) != 2 {
	...
	if tasks[0].Payload != "task1" {
		t.Errorf("wrong order %s", tasks[0].Payload)
	}
	if tasks[1].Payload != "task2" {
...
func TestHandlerPublishReverse(t *testing.T) {
	...
	if tasks[0].Payload != "task2" {
		t.Errorf("wrong order %s", tasks[0].Payload)
	}
	if tasks[1].Payload != "task1" {
		t.Errorf("wrong order %s", tasks[1].Payload)
	}
	time.Sleep(time.Second)
	firstTask, ok := h.Get()
...
func TestHandlerUnInitializedExecuteAt(t *testing.T) {
	...
	h := Handler[time.Time]{}
	_ = h.ExecuteAt(time.Now().Add(time.Minute), time.Now().Add(time.Minute))
}
